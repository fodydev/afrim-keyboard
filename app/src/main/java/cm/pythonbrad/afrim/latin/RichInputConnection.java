...
import cm.pythonbrad.afrim.latin.common.UnicodeSurrogate;
import cm.pythonbrad.afrim.latin.settings.SpacingAndPunctuations;
import cm.pythonbrad.afrim.latin.utils.CapsModeUtils;
import cm.pythonbrad.afrim.latin.utils.DebugLogUtils;

/**
 * Enrichment class for InputConnection to simplify interaction and add functionality.
 *
 * <p>This class serves as a wrapper to be able to simply add hooks to any calls to the underlying
 * InputConnection. It also keeps track of a number of things to avoid having to call upon IPC all
 * the time to find out what text is in the buffer, when we need it to determine caps mode for
 * example.
 */
public final class RichInputConnection {
   ...
   * keep this to compare it to the actual selection start to guess whether the move was caused by a
   * keyboard command or not. It's not really the selection start position: the selection start may
   * not be there yet, and in some cases, it may never arrive there.
   */
  ...
  /**
   * This contains the committed text immediately preceding the cursor and the composing text, if
   * any. It is refreshed when the cursor moves by calling upon the TextView.
   */
  private final StringBuilder mCommittedTextBeforeComposingText = new StringBuilder();

  /**
   * This variable is a temporary object used in {@link #commitText(CharSequence,int)} to avoid
   * object creation.
   */
  ...
  private void checkConsistencyForDebug() {
    ...
    final String reference =
        (beforeCursor.length() <= actualLength)
            ...
            : beforeCursor
                .subSequence(beforeCursor.length() - actualLength, beforeCursor.length())
                .toString();
   ...
   *
   * <p>This should be called when the cursor moved. It's possible that we can't connect to the
   * application when doing this; notably, this happens sometimes during rotation, probably because
   * of a race condition in the framework. In this case, we just can't retrieve the data, so we
   * empty the cache and note that we don't know the new cursor position, and we return false so
   * that the caller knows about this and can retry later.
   *
   ...
   * @return true if we were able to connect to the editor successfully, false otherwise. When this
   *     method returns false, the caches could not be correctly refreshed so they were only reset:
   *     the caller should try again later to return to normal operation.
   */
  public boolean resetCachesUponCursorMoveAndReturnSuccess(
  ...
  private boolean reloadTextCache() {
    ...
    final CharSequence textBeforeCursor =
        getTextBeforeCursorAndDetectLaggyConnection(
  ...
  public void commitText(final CharSequence text, final int newCursorPosition) {
    ...
    if (isConnected()) {
      ...
      final CharacterStyle[] spans =
          mTempObjectForCommitText.getSpans(0, text.length(), CharacterStyle.class);
      for (final CharacterStyle span : spans) {
        ...
        // See cm.pythonbrad.afrim.latin.common.Constants.UnicodeSurrogate
   ...
   *
   * <p>This returns a bit set of TextUtils#CAP_MODE_*, masked by the inputType argument. This
   * method also supports faking an additional space after the string passed in argument, to support
   * cases where a space will be added automatically, like in phantom space state for example. Note
   * that for English, we are using American typography rules (which are not specific to American
   * English, it's just the most common set of rules for English).
   *
  ...
  public int getCursorCapsMode(
      final int inputType, final SpacingAndPunctuations spacingAndPunctuations) {
    mIC = mParent.getCurrentInputConnection();
    if (!isConnected()) {
    ...
    return CapsModeUtils.getCapsMode(
        mCommittedTextBeforeComposingText.toString(), inputType, spacingAndPunctuations);
  }
  ...
  public CharSequence getTextBeforeCursor(final int n, final int flags) {
    ...
    return getTextBeforeCursorAndDetectLaggyConnection(
        OPERATION_GET_TEXT_BEFORE_CURSOR, SLOW_INPUT_CONNECTION_ON_PARTIAL_RELOAD_MS, n, flags);
  }

  public CharSequence getTextAfterCursor(final int n, final int flags) {
    return getTextAfterCursorAndDetectLaggyConnection(
        OPERATION_GET_TEXT_AFTER_CURSOR, SLOW_INPUT_CONNECTION_ON_PARTIAL_RELOAD_MS, n, flags);
  }
  ...
  private void detectLaggyConnection(
      final int operation, final long timeout, final long startTime) {
   ...
   *
   * <p>Calls through to {@link InputConnection#setSelection(int, int)}.
   *
  ...
  public boolean hasCursorPosition() {
    return mExpectedSelStart != INVALID_CURSOR_POSITION
        && mExpectedSelEnd != INVALID_CURSOR_POSITION;
  }

  /**
   * Some chars, such as emoji consist of 2 chars (surrogate pairs). We should treat them as one
   * character.
   */
  public int getUnicodeSteps(int chars, boolean rightSidePointer) {
    int steps = 0;
    if (chars < 0) {
      CharSequence charsBeforeCursor =
          rightSidePointer && hasSelection()
              ? getSelectedText(0)
