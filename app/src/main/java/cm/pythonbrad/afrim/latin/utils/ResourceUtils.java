...
public final class ResourceUtils {
  ...
  public static String getDeviceOverrideValue(
      final Resources res, final int overrideResId, final String defaultValue) {
    ...
    if (overrideValue != null) {
      Log.i(
          ...
          "Find override value:"
              + " resource="
              + res.getResourceEntryName(overrideResId)
              + " build="
  ...
  static class DeviceOverridePatternSyntaxError extends Exception {
    ...
    public DeviceOverridePatternSyntaxError(
        final String message, final String expression, final Throwable throwable) {
   ...
   * "pattern1[:pattern2...] (or an empty string for the default). A pattern is "key=regexp_value"
   * string. The condition matches only if all patterns of the condition are true for the specified
   * key value pairs.
   *
   * <p>For example, "condition,constant" has the following format. -
   * HARDWARE=mako,constantForNexus4 - MODEL=Nexus 4:MANUFACTURER=LGE,constantForNexus4 -
   * ,defaultConstant
   *
   ...
   *     condition matches.
   */
  private static String findConstantForKeyValuePairs(
      final HashMap<String, String> keyValuePairs, final String[] conditionConstantArray) {
  ...
  private static boolean fulfillsCondition(
      final HashMap<String, String> keyValuePairs, final String condition)
  ...
  public static int getDefaultKeyboardHeight(final Resources res) {
    ...
    final float maxKeyboardHeight =
        res.getFraction(R.fraction.config_max_keyboard_height, dm.heightPixels, dm.heightPixels);
    float minKeyboardHeight =
        res.getFraction(R.fraction.config_min_keyboard_height, dm.heightPixels, dm.heightPixels);
    if (minKeyboardHeight < 0.0f) {
      // Specified fraction was negative, so it should be calculated against display
      // width.
      minKeyboardHeight =
          -res.getFraction(R.fraction.config_min_keyboard_height, dm.widthPixels, dm.widthPixels);
    }
  ...
  public static float getFraction(
      final TypedArray a, final int index, final float base, final float defValue) {
    final TypedValue value = a.peekValue(index);
    if (value == null || !isFractionValue(value)) {
      return defValue;
    }
  ...
  public static float getDimensionOrFraction(
      final TypedArray a, final int index, final int base, final float defValue) {
    final TypedValue value = a.peekValue(index);
    if (value == null) {
      return defValue;
    }
  ...
  public static boolean isBrightColor(int color) {
    ...
    int brightness =
        (int) Math.sqrt(rgb[0] * rgb[0] * .241 + rgb[1] * rgb[1] * .691 + rgb[2] * rgb[2] * .068);
    if (brightness >= 210) {
      bright = true;
    }
    return bright;
