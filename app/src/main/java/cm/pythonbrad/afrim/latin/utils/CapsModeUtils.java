...
public final class CapsModeUtils {
   ...
   *
   * <p>This include the Unicode START_PUNCTUATION category, but also some other symbols that are
  ...
  /**
   * Determine what caps mode should be in effect at the current offset in the text. Only the mode
   * bits set in <var>reqModes</var> will be checked. Note that the caps mode flags here are
   * explicitly defined to match those in {@link InputType}.
   *
   * <p>This code is a straight copy of TextUtils.getCapsMode (modulo namespace and formatting
   * issues). This will change in the future as we simplify the code for our use and fix bugs.
   *
   * @param cs The text that should be checked for caps modes.
   * @param reqModes The modes to be checked: may be any combination of {@link
   *     TextUtils#CAP_MODE_CHARACTERS}, {@link TextUtils#CAP_MODE_WORDS}, and {@link
   *     TextUtils#CAP_MODE_SENTENCES}.
   * @param spacingAndPunctuations The current spacing and punctuations settings.
   * @return Returns the actual capitalization modes that can be in effect at the current position,
   *     which is any combination of {@link TextUtils#CAP_MODE_CHARACTERS}, {@link
   *     TextUtils#CAP_MODE_WORDS}, and {@link TextUtils#CAP_MODE_SENTENCES}.
   */
  public static int getCapsMode(
    ...
    if (spacingAndPunctuations.mUsesAmericanTypography) {
      for (; i > 0; i--) {
        ...
        final char c = cs.charAt(i - 1);
        if (c != Constants.CODE_DOUBLE_QUOTE
