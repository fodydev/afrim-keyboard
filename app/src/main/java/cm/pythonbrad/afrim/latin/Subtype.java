...
public final class Subtype {
   ...
   *     used in the main display name of the subtype (eg: "English (US) (QWERTY)" vs "English
   *     (US)").
   * @param resources the resources to use.
   */
  public Subtype(
      final String locale,
      final String layoutSet,
      final int layoutNameRes,
      final boolean showLayoutInName,
   ...
   *     language code, "cc" is a country code.
   * @param layoutSet the keyboard layout set name.
   * @param layoutNameStr the keyboard layout name string to use for display instead of the name of
   *     the language.
   * @param showLayoutInName flag to indicate if the display name of the keyboard layout should be
   *     used in the main display name of the subtype (eg: "English (US) (QWERTY)" vs "English
   *     (US)").
  ...
  /**
   * Get the display name for the subtype. This should be something like "English (US)" or "English
   * (US) (QWERTY)".
   *
    ...
    if (mShowLayoutInName) {
      if (mLayoutNameRes != NO_RESOURCE) {
        return mResources.getString(
            R.string.subtype_generic_layout,
            localeDisplayName,
            mResources.getString(mLayoutNameRes));
      }
      if (mLayoutNameStr != null) {
        return mResources.getString(
            R.string.subtype_generic_layout, localeDisplayName, mLayoutNameStr);
      }
    }
