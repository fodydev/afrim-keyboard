...
package cm.pythonbrad.afrim.latin.inputlogic;
...
import cm.pythonbrad.afrim.latin.LatinIME;
import cm.pythonbrad.afrim.latin.RichInputConnection;
import cm.pythonbrad.afrim.latin.common.Constants;
import cm.pythonbrad.afrim.latin.common.StringUtils;
import cm.pythonbrad.afrim.latin.settings.SettingsValues;
import cm.pythonbrad.afrim.latin.utils.InputTypeUtils;
import cm.pythonbrad.afrim.latin.utils.RecapitalizeStatus;
import cm.pythonbrad.afrim.latin.utils.SubtypeLocaleUtils;
...
public final class InputLogic {
   ...
   *
   * <p>Call this when input starts or restarts in some editor (typically, in onStartInputView).
   */
   ...
   *
   * <p>This is triggered by keys that input many characters at once, like the ".com" key or some
   * additional keys for example.
   *
  ...
  /**
   * Consider an update to the cursor position. Evaluate whether this update has happened as part of
   * normal typing or whether it was an explicit cursor move by the user. In any case, do the
   ...
   *
   * <p>Typically, this is called whenever a key is pressed on the software keyboard. This is not
   ...
   *
   * <p>Consumed events represent events that have already been consumed, typically by the combining
   * chain.
   *
   * @param event The event to handle.
   */
   ...
   *
   * <p>A functional event is a special key, like delete, shift, emoji, or the settings key.
   * Non-special keys are those that generate a single code point. This includes all letters,
   * digits, punctuation, separators, emoji. It excludes keys that manage keyboard-related stuff
   * like shift, language switch, settings, layout switch, or any key that results in multiple code
   * points like the ".com" key.
   *
   * @param event The event to handle.
   ...
   *
   * <p>These events are generally events that cause input, but in some cases they may do other
   * things like trigger an editor action.
   *
   * @param event The event to handle.
   * @param inputTransaction The transaction in progress.
   */
  private void handleNonFunctionalEvent(
      final Event event, final InputTransaction inputTransaction) {
   ...
   *
   * <p>Non-special keys are those that generate a single code point. This includes all letters,
   * digits, punctuation, separators, emoji. It excludes keys that manage keyboard-related stuff
   * like shift, language switch, settings, layout switch, or any key that results in multiple code
   * points like the ".com" key.
   *
   ...
   * @param inputTransaction The transaction in progress.
   */
  private void handleNonSpecialCharacterEvent(
      final Event event, final InputTransaction inputTransaction) {
  ...
  private void performRecapitalization(final SettingsValues settingsValues) {
    ...
    mConnection.replaceText(
        selectionStart, selectionEnd, mRecapitalizeStatus.getRecapitalizedString());
    mConnection.setSelection(
        mRecapitalizeStatus.getNewCursorStart(), mRecapitalizeStatus.getNewCursorEnd());
   ...
   *
   * <p>This method tries its best to do this in the most efficient possible manner. It avoids
   * getting text from the editor if possible at all. This is called from the KeyboardSwitcher
   * (through a trampoline in LatinIME) because it needs to know auto caps state to display the
   * right layout.
   *
  ...
  public int getCurrentAutoCapsState(
      final SettingsValues settingsValues, final String layoutSetName) {
   ...
   *
   * <p>Some keys input a TLD (specifically, the ".com" key) and this warrants some specific
   * processing. First, if this is a TLD, we ignore PHANTOM spaces -- this is done by type of
   * character in onCodeInput, but since this gets inputted as a whole string we need to do it here
   * specifically. Then, if the last character before the cursor is a period, then we cut the dot at
   * the start of ".com". This is because humans tend to type "www.google." and then press the
   * ".com" key and instinctively don't expect to get "www.google..com".
   *
  ...
  private String performSpecificTldProcessingOnTextInput(final String text) {
    if (text.length() <= 1
        || text.charAt(0) != Constants.CODE_PERIOD
   ...
   *
   * <p>This will clear the composing word, reset the last composed word, clear the suggestion strip
   * and tell the input connection about it so that it can refresh its caches.
   *
   ...
   *
   * <p>If possible at all, avoid using this method. It causes all sorts of race conditions with the
   * text view because it goes through a different, asynchronous binder. Also, batch edits are
   * ignored for key events. Use the normal software input methods instead.
   *
   ...
   *
   * <p>Normally we send code points with commitText, but there are some cases (where backward
   ...
   *
   * <p>When the editor can't be accessed we can't reset the caches, so we schedule a retry. This
   * method handles the retry, and re-schedules a new retry if we still can't access. We only retry
  ...
  public boolean retryResetCachesAndReturnSuccess(
      final boolean tryResumeSuggestions,
