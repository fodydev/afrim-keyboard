...
import cm.pythonbrad.afrim.latin.Subtype;
import cm.pythonbrad.afrim.latin.common.LocaleUtils;
...
public final class SubtypeLocaleUtils {
  ...
  private static final String[] sSupportedLocales =
      new String[] {
  ...
  public static Subtype getSubtype(
      final String locale, final String layoutSet, final Resources resources) {
    final List<Subtype> subtypes = new SubtypeBuilder(locale, layoutSet, resources).getSubtypes();
    return subtypes.size() == 0 ? null : subtypes.get(0);
  }
  ...
  /**
   * Utility for building the supported subtype objects. {@link #getSubtypes} sets up the full list
   * of available subtypes for a locale, but not all of the subtypes that it requests always get
   * returned. The parameters passed in the constructor limit what subtypes are actually built and
   * returned. This allows for a central location for indicating what subtypes are available for
   * each locale without always needing to build them all.
   */
  private static class SubtypeBuilder {
     ...
     * @param resources the resources to use.
     */
    public SubtypeBuilder(final String locale, final String layoutSet, final Resources resources) {
      mLocale = locale;
    ...
    private void addLayout(final String keyboardLayoutSet) {
      ...
      if (predefinedLayoutIndex >= 0) {
        final String[] predefinedLayoutDisplayNames =
            mResources.getStringArray(R.array.predefined_layout_display_names);
        layoutNameStr = predefinedLayoutDisplayNames[predefinedLayoutIndex];
      } else {
        layoutNameStr = null;
      }

      mSubtypes.add(new Subtype(mLocale, keyboardLayoutSet, layoutNameStr, false, mResources));
    }
    ...
    private void addLayout(final String keyboardLayoutSet, final int layoutRes) {
      ...
      mSubtypes.add(new Subtype(mLocale, keyboardLayoutSet, layoutRes, true, mResources));
    }

    /**
     * Add the predefined layouts (eg: QWERTY, AZERTY, etc) for the locale. This might not actually
     * add all of the subtypes to the list depending on the original request.
     */
    private void addGenericLayouts() {
      ...
      final String[] predefinedKeyboardLayoutSets =
          mResources.getStringArray(R.array.predefined_layouts);
      final String[] predefinedKeyboardLayoutSetDisplayNames =
          mResources.getStringArray(R.array.predefined_layout_display_names);
      for (int i = 0; i < predefinedKeyboardLayoutSets.length; i++) {
        ...
        mSubtypes.add(
            new Subtype(
                mLocale,
                predefinedLayout,
                predefinedKeyboardLayoutSetDisplayNames[i],
                true,
                mResources));
      }
    }
