...
import cm.pythonbrad.afrim.keyboard.KeyboardId;
import cm.pythonbrad.afrim.keyboard.KeyboardTheme;
import cm.pythonbrad.afrim.latin.common.StringUtils;
import cm.pythonbrad.afrim.latin.utils.ResourceUtils;
import cm.pythonbrad.afrim.latin.utils.XmlParseUtils;
import cm.pythonbrad.afrim.latin.utils.XmlParseUtils.ParseException;
 ...
 *
 * <p>This class parses Keyboard XML file and eventually build a Keyboard. The Keyboard XML file
 * looks like:
 *
 * <pre>
 *   &lt;!-- xml/keyboard.xml --&gt;
...
public class KeyboardBuilder<KP extends KeyboardParams> {
          ...
          if (!skip) {
            if (mKeyboardDefined) {
              throw new XmlParseUtils.ParseException(
                  "Only one " + TAG_KEYBOARD + " tag can be defined", parser);
            }
  ...
  private void parseKeyboardAttributes(final XmlPullParser parser) {
    ...
    try {
      ...
      final int bonusHeight =
          (int) keyboardAttr.getFraction(R.styleable.Keyboard_bonusHeight, height, height, 0);
      ...
      params.mHorizontalGap =
          keyboardAttr.getFraction(R.styleable.Keyboard_horizontalGap, width, width, 0);
      final float baseWidth =
          params.mOccupiedWidth
              - params.mLeftPadding
              - params.mRightPadding
              + params.mHorizontalGap;
      params.mBaseWidth = baseWidth;
      params.mDefaultKeyPaddedWidth =
      ...
      // paddings.
      params.mVerticalGap =
          keyboardAttr.getFraction(R.styleable.Keyboard_verticalGap, height, height, 0);
  ...
  private void parseKey(final XmlPullParser parser, final KeyboardRow row, final boolean skip)
    ...
    final TypedArray keyAttr =
        mResources.obtainAttributes(Xml.asAttributeSet(parser), R.styleable.Keyboard_Key);
  ...
  private void parseIncludeRowContent(
      final XmlPullParser parser, final KeyboardRow row, final boolean skip)
      throws XmlPullParserException, IOException {
  ...
  private void parseIncludeInternal(
    ...
    final TypedArray includeAttr = mResources.obtainAttributes(attr, R.styleable.Keyboard);
  ...
  private void parseSwitchRowContent(
      final XmlPullParser parser, final KeyboardRow row, final boolean skip)
      throws XmlPullParserException, IOException {
    parseSwitchInternal(parser, false, row, skip);
  }

  private void parseSwitchInternal(
      final XmlPullParser parser,
      final boolean parseKeyboard,
  ...
  private boolean parseCaseCondition(final XmlPullParser parser) {
    ...
    try {
      ...
      final boolean keyboardThemeMatched =
          matchTypedValue(
              caseAttr,
  ...
  private static boolean isIconDefined(
      final TypedArray a, final int index, final KeyboardIconsSet iconsSet) {
    if (!a.hasValue(index)) {
      return true;
    }
