...
import cm.pythonbrad.afrim.keyboard.KeyDetector;
import cm.pythonbrad.afrim.keyboard.PointerTracker;
...
public final class NonDistinctMultitouchHelper {
  ...
  public void processMotionEvent(final MotionEvent me, final KeyDetector keyDetector) {
    ...
    if (oldPointerCount == 1 && pointerCount == 1) {
      ...
      injectMotionEvent(
          action, me.getX(index), me.getY(index), downTime, eventTime, mainTracker, keyDetector);
      return;
    }
    ...
    if (oldPointerCount == 2 && pointerCount == 1) {
      ...
      if (mOldKey != newKey) {
        ...
        injectMotionEvent(
            MotionEvent.ACTION_DOWN, x, y, downTime, eventTime, mainTracker, keyDetector);
        if (action == MotionEvent.ACTION_UP) {
          // Inject an artifact up event for the new key also.
          injectMotionEvent(
              MotionEvent.ACTION_UP, x, y, downTime, eventTime, mainTracker, keyDetector);
        }
      }
      return;
    }
    ...
    Log.w(
        TAG,
        "Unknown touch panel behavior: pointer count is "
            + pointerCount
            + " (previously "
            + oldPointerCount
            + ")");
  ...
  private static void injectMotionEvent(
    ...
    final MotionEvent me = MotionEvent.obtain(downTime, eventTime, action, x, y, 0 /* metaState */);
    try {
