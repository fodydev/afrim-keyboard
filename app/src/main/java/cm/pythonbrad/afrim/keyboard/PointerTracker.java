...
import cm.pythonbrad.afrim.keyboard.internal.BogusMoveEventDetector;
import cm.pythonbrad.afrim.keyboard.internal.DrawingProxy;
import cm.pythonbrad.afrim.keyboard.internal.PointerTrackerQueue;
import cm.pythonbrad.afrim.keyboard.internal.TimerProxy;
import cm.pythonbrad.afrim.latin.common.Constants;
import cm.pythonbrad.afrim.latin.common.CoordinateUtils;
import cm.pythonbrad.afrim.latin.define.DebugFlags;
import cm.pythonbrad.afrim.latin.settings.Settings;
...
public final class PointerTracker implements PointerTrackerQueue.Element {
  ...
  static final class PointerTrackerParams {
    ...
    public PointerTrackerParams(final TypedArray mainKeyboardViewAttr) {
      mKeySelectionByDraggingFinger =
          mainKeyboardViewAttr.getBoolean(
              R.styleable.MainKeyboardView_keySelectionByDraggingFinger, false);
      mTouchNoiseThresholdTime =
          mainKeyboardViewAttr.getInt(R.styleable.MainKeyboardView_touchNoiseThresholdTime, 0);
      mTouchNoiseThresholdDistance =
          mainKeyboardViewAttr.getDimensionPixelSize(
              R.styleable.MainKeyboardView_touchNoiseThresholdDistance, 0);
      mKeyRepeatStartTimeout =
          mainKeyboardViewAttr.getInt(R.styleable.MainKeyboardView_keyRepeatStartTimeout, 0);
      mKeyRepeatInterval =
          mainKeyboardViewAttr.getInt(R.styleable.MainKeyboardView_keyRepeatInterval, 0);
      mLongPressShiftLockTimeout =
          mainKeyboardViewAttr.getInt(R.styleable.MainKeyboardView_longPressShiftLockTimeout, 0);
    }
  }
  ...
  private static int sPointerStep =
      (int) (10.0 * Resources.getSystem().getDisplayMetrics().density);
  ...
  private boolean callListenerOnPressAndCheckKeyboardLayoutChange(
      final Key key, final int repeatCount) {
  ...
  private void callListenerOnCodeInput(
      final Key key, final int primaryCode, final int x, final int y, final boolean isKeyRepeat) {
    final boolean ignoreModifierKey = mIsInDraggingFinger && key.isModifier();
    final boolean altersCode = key.altCodeWhileTyping() && sTimerProxy.isTypingState();
    final int code = altersCode ? key.getAltCode() : primaryCode;
    if (DEBUG_LISTENER) {
      final String output =
          code == Constants.CODE_OUTPUT_TEXT ? key.getOutputText() : Constants.printableCode(code);
      Log.d(
          TAG,
          String.format(
              "[%d] onCodeInput: %4d %4d %s%s%s",
              mPointerId,
              x,
              y,
              output,
    ...
    if (code == Constants.CODE_OUTPUT_TEXT) {
      ...
      sListener.onCodeInput(
          code, Constants.NOT_A_COORDINATE, Constants.NOT_A_COORDINATE, isKeyRepeat);
    }
  }

  // Note that we need primaryCode argument because the keyboard may be in shifted state and the
  // primaryCode is different from {@link Key#mKeyCode}.
  private void callListenerOnRelease(
      final Key key, final int primaryCode, final boolean withSliding) {
  ...
  private void setKeyDetectorInner(final KeyDetector keyDetector) {
    ...
    final int keyPaddedWidth = mKeyboard.mMostCommonKeyWidth + Math.round(mKeyboard.mHorizontalGap);
    final int keyPaddedHeight = mKeyboard.mMostCommonKeyHeight + Math.round(mKeyboard.mVerticalGap);
  ...
  private void onDownEvent(
      final int x, final int y, final long eventTime, final KeyDetector keyDetector) {
      ...
      if (distance < sParams.mTouchNoiseThresholdDistance) {
        if (DEBUG_MODE)
          Log.w(
              TAG,
              String.format(
                  "[%d] onDownEvent:" + " ignore potential noise: time=%d distance=%d",
  ...
  private void onDownEventInternal(final int x, final int y) {
    ...
    mIsAllowedDraggingFinger =
        sParams.mKeySelectionByDraggingFinger
  ...
  private void dragFingerFromOldKeyToNewKey(
      final Key key, final int x, final int y, final long eventTime, final Key oldKey) {
    ...
    // Caveat: When in chording input mode with a modifier key, we don't use this hack.
      ...
      if (DEBUG_MODE) {
        Log.w(
            TAG,
            String.format(
                "[%d] onMoveEvent:" + " detected sliding finger while multi touching", mPointerId));
      }
      onUpEvent(x, y, eventTime);
      cancelTrackingForAction();
      setReleasedKeyGraphics(oldKey, true /* withAnimation */);
  ...
  private void onUpEventInternal(final int x, final int y) {
    ...
    if (currentKey != null
        && currentKey.isRepeatable()
        && (currentKey.getCode() == currentRepeatingKeyCode)
        && !isInDraggingFinger) {
      return;
    }
  ...
  private boolean isMajorEnoughMoveToBeOnNewKey(final int x, final int y, final Key newKey) {
    ...
    final int keyHysteresisDistanceSquared =
        mKeyDetector.getKeyHysteresisDistanceSquared(mIsInSlidingKeyInput);
    final int distanceFromKeyEdgeSquared = curKey.squaredDistanceToHitboxEdge(x, y);
    if (distanceFromKeyEdgeSquared >= keyHysteresisDistanceSquared) {
      if (DEBUG_MODE) {
        final float distanceToEdgeRatio =
            (float) Math.sqrt(distanceFromKeyEdgeSquared)
                / (mKeyboard.mMostCommonKeyWidth + mKeyboard.mHorizontalGap);
        Log.d(
            TAG,
            String.format(
                "[%d] isMajorEnoughMoveToBeOnNewKey:" + " %.2f key width from key edge",
  ...
  private void printTouchEvent(final String title, final int x, final int y, final long eventTime) {
    ...
    Log.d(
        ...
        String.format(
            "[%d]%s%s %4d %4d %5d %s",
            mPointerId, (mIsTrackingForActionDisabled ? "-" : " "), title, x, y, eventTime, code));
  }
}
