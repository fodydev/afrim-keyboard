...
public interface KeyboardActionListener {
  /**
   * Called when the user presses a key. This is sent before the {@link #onCodeInput} is called. For
  ...
  /**
   * Called when the user releases a key. This is sent after the {@link #onCodeInput} is called. For
   * keys that repeat, this is only called once.
   *
   * @param primaryCode the code of the key that was released
   * @param withSliding true if releasing has occurred because the user slid finger from the key to
   *     other key without releasing the finger.
   */
   ...
   * @param x x-coordinate pixel of touched event. If {@link #onCodeInput} is not called by {@link
   *     PointerTracker} or so, the value should be {@link Constants#NOT_A_COORDINATE}. If it's
   *     called on insertion from the suggestion strip, it should be {@link
   *     Constants#SUGGESTION_STRIP_COORDINATE}.
   * @param y y-coordinate pixel of touched event. If {@link #onCodeInput} is not called by {@link
   *     PointerTracker} or so, the value should be {@link Constants#NOT_A_COORDINATE}.If it's
   *     called on insertion from the suggestion strip, it should be {@link
   *     Constants#SUGGESTION_STRIP_COORDINATE}.
