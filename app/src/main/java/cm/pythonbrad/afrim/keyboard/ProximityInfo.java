...
public class ProximityInfo {
  ...
  private void computeNearestNeighbors() {
    ...
    for (final Key key : mSortedKeys) {
      ...
      final int keyBottom = Math.min(keyY + key.getHeight() + key.getBottomPadding(), maxKeyBottom);
      final int keyLeft = keyX - key.getLeftPadding();
      final int keyRight = Math.min(keyX + key.getWidth() + key.getRightPadding(), maxKeyRight);
