 ...
 *
 * <p>Each key specification is one of the following: - Label optionally followed by keyOutputText
 * (keyLabel|keyOutputText). - Label optionally followed by code point (keyLabel|!code/code_name). -
 * Icon followed by keyOutputText (!icon/icon_name|keyOutputText). - Icon followed by code point
 * (!icon/icon_name|!code/code_name). Label and keyOutputText are one of the following: - Literal
 * string. - Label reference represented by (!text/label_name), see {@link KeyboardTextsSet}. -
 * String resource reference represented by (!text/resource_name), see {@link KeyboardTextsSet}.
 * Icon is represented by (!icon/icon_name), see {@link KeyboardIconsSet}. Code is one of the
 * following: - Code point presented by hexadecimal string prefixed with "0x" - Code reference
 * represented by (!code/code_name), see {@link KeyboardCodesSet}. Special character, comma ','
 * backslash '\', and bar '|' can be escaped by '\' character. Note that the '\' is also parsed by
 * XML parser and {@link MoreKeySpec#splitKeySpecs(String)} as well.
 */
...
public final class KeySpecParser {
  ...
  public static int getIconId(final String keySpec) {
    ...
    final String iconName =
        getBeforeLabelEnd(keySpec, labelEnd).substring(KeyboardIconsSet.PREFIX_ICON.length());
