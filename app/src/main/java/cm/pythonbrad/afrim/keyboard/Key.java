...
package cm.pythonbrad.afrim.keyboard;

import static cm.pythonbrad.afrim.latin.common.Constants.CODE_OUTPUT_TEXT;
import static cm.pythonbrad.afrim.latin.common.Constants.CODE_SHIFT;
import static cm.pythonbrad.afrim.latin.common.Constants.CODE_SWITCH_ALPHA_SYMBOL;
import static cm.pythonbrad.afrim.latin.common.Constants.CODE_UNSPECIFIED;
...
import cm.pythonbrad.afrim.keyboard.internal.KeyDrawParams;
import cm.pythonbrad.afrim.keyboard.internal.KeySpecParser;
import cm.pythonbrad.afrim.keyboard.internal.KeyStyle;
import cm.pythonbrad.afrim.keyboard.internal.KeyVisualAttributes;
import cm.pythonbrad.afrim.keyboard.internal.KeyboardIconsSet;
import cm.pythonbrad.afrim.keyboard.internal.KeyboardParams;
import cm.pythonbrad.afrim.keyboard.internal.KeyboardRow;
import cm.pythonbrad.afrim.keyboard.internal.MoreKeySpec;
...
public class Key implements Comparable<Key> {
  ...
  private static final int LABEL_FLAGS_AUTO_SCALE =
      LABEL_FLAGS_AUTO_X_SCALE | LABEL_FLAGS_AUTO_Y_SCALE;
  ...
  /**
   * X coordinate of the top-left corner of the key in the keyboard layout, excluding the padding.
   */
  private final int mX;

  /**
   * Y coordinate of the top-left corner of the key in the keyboard layout, excluding the padding.
   */
  ...
  public Key(
    ...
    mHitbox.set(
        Math.round(x - leftPadding),
        Math.round(y - topPadding),
        Math.round(x + width + rightPadding),
        Math.round(y + height + bottomPadding));
    mX = Math.round(x);
    mY = Math.round(y);
  ...
  public Key(
    ...
    mHitbox.set(
        Math.round(keyLeft - leftPadding),
        Math.round(keyTop - topPadding),
        Math.round(keyRight + rightPadding),
        Math.round(keyBottom + bottomPadding));
    ...
    mLabelFlags =
        style.getFlags(keyAttr, R.styleable.Keyboard_Key_keyLabelFlags)
    ...
    if ((value = MoreKeySpec.getIntValue(moreKeys, MORE_KEYS_AUTO_COLUMN_ORDER, -1)) > 0) {
      ...
      moreKeysColumnAndFlags =
          MORE_KEYS_MODE_FIXED_COLUMN_WITH_AUTO_ORDER | (value & MORE_KEYS_COLUMN_NUMBER_MASK);
    }
    if ((value = MoreKeySpec.getIntValue(moreKeys, MORE_KEYS_FIXED_COLUMN_ORDER, -1)) > 0) {
      // Override with fixed column order number and set a relevant mode value.
      moreKeysColumnAndFlags =
          MORE_KEYS_MODE_FIXED_COLUMN_WITH_FIXED_ORDER | (value & MORE_KEYS_COLUMN_NUMBER_MASK);
    }
  ...
  public static Key removeRedundantMoreKeys(
      final Key key, final MoreKeySpec.LettersOnBaseLayout lettersOnBaseLayout) {
    final MoreKeySpec[] moreKeys = key.getMoreKeys();
    final MoreKeySpec[] filteredMoreKeys =
        MoreKeySpec.removeRedundantMoreKeys(moreKeys, lettersOnBaseLayout);
  ...
  public final int selectHintTextColor(final KeyDrawParams params) {
    ...
    if (hasShiftedLetterHint()) {
      return isShiftedLetterActivated()
  ...
  /**
   * Gets the theoretical width of the key in pixels, excluding the padding. This is the exact width
   * that the key was defined to be, but this will likely differ from the actual drawn width because
   * the normal (drawn/functional) width was determined by rounding the left and right edge to fit
   * evenly in a pixel.
   *
   ...
   * height that the key was defined to be, but this will likely differ from the actual drawn height
   * because the normal (drawn/functional) width was determined by rounding the top and bottom edge
   * to fit evenly in a pixel.
   *
   * @return The defined width of the key in pixels, excluding the padding.
   */
  ...
  /**
   * Informs the key that it has been released, in case it needs to change its appearance or state.
   *
   * @see #onPressed()
   */
   ...
   * @return whether or not the point falls on the key. This generally includes all points between
   *     the key and the keyboard edge for keys attached to an edge and all points between the key
   *     and halfway to adjacent keys.
   */
   ...
   */
  public final Drawable selectBackgroundDrawable(
