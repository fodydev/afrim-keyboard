...
import cm.pythonbrad.afrim.keyboard.internal.KeyboardBuilder;
...
import cm.pythonbrad.afrim.latin.utils.TypefaceUtils;
...
public final class MoreKeysKeyboard extends Keyboard {
  ...
  static class MoreKeysKeyboardParams extends KeyboardParams {
     ...
     * @param isMoreKeysFixedColumn true if more keys keyboard should have <code>numColumn</code>
     *     columns. Otherwise more keys keyboard should have <code>numColumn</code> columns at most.
     * @param isMoreKeysFixedOrder true if the order of more keys is determined by the order in the
     *     more keys' specification. Otherwise the order of more keys is automatically determined.
     */
    public void setParameters(
        final int numKeys,
        final int numColumn,
        final float keyPaddedWidth,
        final float rowHeight,
        final float coordXInParent,
        final int parentKeyboardWidth,
      ...
      final float availableWidth =
          parentKeyboardWidth - mLeftPadding - mRightPadding + mHorizontalGap;
      ...
      if (isMoreKeysFixedColumn) {
        ...
        if (maxColumns < requestedNumColumns) {
          Log.e(
              TAG,
              "Keyboard is too small to hold the requested more keys columns: "
                  + availableWidth
                  + " "
                  + keyPaddedWidth
                  + " "
      ...
      final float leftWidth =
          Math.max(coordXInParent - mLeftPadding - keyPaddedWidth / 2 + mHorizontalGap / 2, 0);
    ...
    private static int getOptimizedColumns(
        final int numKeys, final int maxColumns, final int numRows) {
  ...
  public static class Builder extends KeyboardBuilder<MoreKeysKeyboardParams> {
     ...
     * @param isSingleMoreKeyWithPreview true if the <code>key</code> has just a single "more key"
     *     and its key popup preview is enabled.
    ...
    public Builder(
        ...
        final int keyPreviewVisibleWidth,
        final int keyPreviewVisibleHeight,
      ...
      if (isSingleMoreKeyWithPreview) {
        ...
        final float keyboardHorizontalPadding = mParams.mLeftPadding + mParams.mRightPadding;
        ...
        final float defaultKeyWidth = mParams.mDefaultKeyPaddedWidth - mParams.mHorizontalGap;
        final float padding =
            context
                    .getResources()
                    .getDimension(R.dimen.config_more_keys_keyboard_key_horizontal_padding)
                + (key.hasLabelsInMoreKeys() ? defaultKeyWidth * LABEL_PADDING_RATIO : 0.0f);
        keyPaddedWidth =
            getMaxKeyWidth(key, defaultKeyWidth, padding, paintToMeasure) + mParams.mHorizontalGap;
      ...
      mParams.setParameters(
          ...
          keyboard.mId.mWidth,
          key.isMoreKeysFixedColumn(),
          key.isMoreKeysFixedOrder());
    }

    private static float getMaxKeyWidth(
        final Key parentKey, final float minKeyWidth, final float padding, final Paint paint) {
      float maxWidth = minKeyWidth;
      for (final MoreKeySpec spec : parentKey.getMoreKeys()) {
        final String label = spec.mLabel;
        // If the label is single letter, minKeyWidth is enough to hold the label.
        if (label != null && StringUtils.codePointCount(label) > 1) {
          maxWidth = Math.max(maxWidth, TypefaceUtils.getStringWidth(label, paint) + padding);
        }
      }
      return maxWidth;
    }
