...
/**
 * This class controls pop up key previews. This class decides: - what kind of key previews should
 * be shown. - where key previews should be placed. - how key previews should be shown and
 * dismissed.
 */
public final class KeyPreviewChoreographer {
  ...
  private void placeKeyPreview(
    ...
    int previewX = key.getX() - (previewWidth - keyWidth) / 2 + CoordinateUtils.x(originCoords);
    // The key preview is placed vertically above the top edge of the parent key with an
    // arbitrary offset.
    final int previewY =
        key.getY() - previewHeight + mParams.mPreviewOffset + CoordinateUtils.y(originCoords);
  ...
  void showKeyPreview(
      final Key key, final KeyPreviewView keyPreviewView, final boolean withAnimation) {
    if (!withAnimation) {
