...
import cm.pythonbrad.afrim.latin.common.CollectionUtils;
 ...
 *
 * <p>The more keys specification is comma separated "key specification" each of which represents
 * one "more key". The key specification might have label or string resource reference in it. These
 * references are expanded before parsing comma. Special character, comma ',' backslash '\' can be
 * escaped by '\' character. Note that the '\' is also parsed by XML parser and {@link
 * MoreKeySpec#splitKeySpecs(String)} as well.
 */
...
public final class MoreKeySpec {
  ...
  public MoreKeySpec(final String moreKeySpec, boolean needsToUpperCase, final Locale locale) {
    ...
    final int code =
        needsToUpperCase ? StringUtils.toTitleCaseOfKeyCode(codeInSpec, locale) : codeInSpec;
    if (code == Constants.CODE_UNSPECIFIED) {
      ...
      mOutputText =
          needsToUpperCase ? StringUtils.toTitleCaseOfKeyLabel(outputText, locale) : outputText;
    }
  ...
  public String toString() {
    ...
    final String output =
        (mCode == Constants.CODE_OUTPUT_TEXT ? mOutputText : Constants.printableCode(mCode));
  ...
  public static MoreKeySpec[] removeRedundantMoreKeys(
      final MoreKeySpec[] moreKeys, final LettersOnBaseLayout lettersOnBaseLayout) {
    if (moreKeys == null) {
      return null;
    }
  ...
  public static String[] insertAdditionalMoreKeys(
      final String[] moreKeySpecs, final String[] additionalMoreKeySpecs) {
  ...
  public static int getIntValue(final String[] moreKeys, final String key, final int defaultValue) {
    if (moreKeys == null) {
      return defaultValue;
    }
