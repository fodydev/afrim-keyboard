...
public final class KeyboardTextsTable {
  ...
  private static final String[] TEXTS_ar = {
    ...
    // In order to make Tatweel easily distinguishable from other punctuations, we use consecutive
    // Tatweels only for its displayed label.
    // Note: The space character is needed as a preceding letter to draw Arabic diacritics
  ...
  static {
    ...
    for (int i = 0; i < LOCALES_AND_TEXTS.length; i += 2) {
      ...
      //    Log.i("LocaleIndexCheck", textToCheck + " is used in " + locale + " total length: " +
      // textsTable.length);
    }
  }
}
